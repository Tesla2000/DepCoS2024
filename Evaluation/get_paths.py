...
def get_paths(
    ...
    if num_splits == 1:
        ...
        cv_iterable = skf.split(patients_ids, [int(patient_id not in healthy_patient_ids) for patient_id in patients_ids])
        next(cv_iterable)
    else:
        skf = StratifiedKFold(
            n_splits=num_splits, shuffle=True, random_state=random_state
        )
        cv_iterable = skf.split(patients_ids, [int(patient_id not in Config.healthy_patients_folder.iterdir()) for patient_id in patients_ids])
